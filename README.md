...
## 2026-06-22 Bot1 四小时内容排期（本地待部署）

本地代码和测试已按下列北京时间排期完成，尚未提交、推送或部署，线上仍保持原状：

| 北京时间 | 栏目 | 发送方式 |
|---|---|---|
| 00:17 | 普通行情 | 静默 |
| 04:17 | 普通行情 | 静默 |
| 08:17 | 早间市场简报 | 正常通知，替代普通行情 |
| 12:17 | 普通行情 | 正常通知 |
| 16:17 | 普通行情 | 正常通知 |
| 20:17（周一至周六） | 晚间重点观察 | 正常通知，替代普通行情 |
| 20:17（周日） | 每周市场复盘 | 正常通知，替代晚间观察和普通行情 |

实现说明：

- 每个时段只生成一个栏目，特殊栏目不会再叠加普通行情。
- 真实发送继续要求内部接口密钥和 `ALLOW_REAL_BROADCAST=true` 双门控，并受北京时间 `:10-:45` 代码窗口保护。
- 同一进程内按“栏目 + 日期 + 小时”去重；dry-run 不发送、不写数据库、不占用时段。
- dry-run 返回 `messages[].text` 完整正文和 `messages[].silent` 静默标记。
- 早间简报展示全球总市值、24 小时变化、BTC 占有率、市值前 100 涨跌分布、主流币、领涨、回撤和热门关注，并只用数据规则生成市场状态。
- 晚间观察聚焦当前强弱、市场宽度、BTC 占有率、BTC/ETH/SOL 及领涨回撤，不复制早报结构；周报展示 BTC/ETH/SOL 当前价格和 7 日表现，并过滤稳定币、包装币和重复锚定资产。
- 三类栏目继续复用 CoinGecko 免费接口、缓存、格式和发送链路；不添加新闻、事件或行情原因推测。
- GitHub Actions 自动 schedule 继续停用；Render、cron-job.org、`.env` 和密钥本次均未修改。
- 异动雷达已完成本地 dry-run 版本：BTC/ETH 1 小时阈值为 ±2.5%，其他白名单主流币为 ±4%，同币同方向冷却 2 小时，全频道每小时最多 3 条。
- 雷达消息会把 CoinGecko `last_updated` 统一转换为北京时间，并显示“数据时间（北京时间）”。
- 雷达状态当前只保存在进程内，且 `RADAR_PRODUCTION_ENABLED=false` 硬锁默认禁止真实发送；未接入排期或生产入口，必须先完成持久化再评估上线。
- 持久化结论和生产准入条件见 `docs/RADAR_PERSISTENCE_EVALUATION.md`。

### 上线前所需 cron 调整

当前主触发来自 Render 内部广播接口。上线时应在 cron-job.org 将原每两小时触发改为北京时间 `00:17、04:17、08:17、12:17、16:17、20:17`。如果平台使用 UTC，则对应 `16:17、20:17（前一日）、00:17、04:17、08:17、12:17`；等价 UTC cron 为 `17 */4 * * *`。本次只记录清单，没有修改线上任务。

## 项目概述

这是一个零成本的加密货币行情Telegram机器人系统，采用双Bot架构：
- **1号Bot（广播员）**：通过 Render 内部广播接口定时推送行情；GitHub Actions 仅保留手动测试备用
- **2号Bot（互动客服）**：处理群组指令查询（Render免费版）

## P0 稳定试运营状态

截至 2026-06-12，Crypto Pulse 已完成 P0 稳定试运营第一阶段验证。
