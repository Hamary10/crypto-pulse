...
`generate_report.py` 默认读取本地 `crypto_pulse.db`。如果本地没有该文件，脚本会生成“未找到数据库文件”的友好报告。这表示本地没有运营数据，不表示报告脚本异常。

二、数据现在在哪里

- Bot2 的真实用户数据、命令记录和币种查询统计目前在 Render 运行环境里的 SQLite 数据库中。
- Bot1 的数据在 GitHub Actions 临时环境中，不适合作长期统计。
- 本地电脑默认没有线上数据库，除非手动取得数据库备份。
- Render 免费环境 SQLite 不适合长期稳定保存，可能在重启、重新部署或实例迁移时丢失。

三、我现在应该怎么看运营情况

P0 开放测试阶段先通过 Render Logs 和 Telegram 群实际使用情况观察：

1. Render Logs 中是否出现 `Received command`。
2. Render Logs 中是否出现 `SQLite command logged`。
3. Render Logs 中是否出现 `SQLite coin query updated`。
4. Telegram 群里是否有真实用户使用 Bot 命令。
