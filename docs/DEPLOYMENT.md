...
## 2026-06-22 四小时排期部署清单（尚未执行）

本节记录本地已完成、生产尚未执行的变更。不得把它误写成已上线：

1. cron-job.org 将原每两小时触发改为北京时间 `00:17、04:17、08:17、12:17、16:17、20:17`。
2. 若调度器使用 UTC，则设为 `16:17、20:17（北京时间前一日对应）、00:17、04:17、08:17、12:17`，等价表达式为 `17 */4 * * *`。
3. 请求目标仍是受保护的 Render 内部广播接口；请求头密钥和值不写入文档。
4. 先使用默认 `dry_run=true` 逐个验证普通行情、早间简报、晚间观察、周日周报和窗口边界。
5. 核对 00/04 的 `messages[].silent=true`，其他栏目为 `false`。
6. 审查通过后再部署代码和调整 cron；本地完成不等于生产已上线。

异动雷达不属于本次 cron 调整范围。当前代码没有为雷达增加定时任务、内部接口或生产环境变量，并由 `RADAR_PRODUCTION_ENABLED=false` 阻断真实发送。由于冷却和每小时配额仅保存在单进程内存中，进程重启、重新部署或多实例都会失去一致性，因此当前状态明确为“本地待部署、禁止生产启用”。持久化评估见 `docs/RADAR_PERSISTENCE_EVALUATION.md`。

固定栏目保护保持不变：内部密钥校验、`ALLOW_REAL_BROADCAST=true` 环境门控、北京时间 `:10-:45` 代码窗口、异步锁、进程内时段去重、Telegram uncertain 状态和 CoinGecko 失败保护。
