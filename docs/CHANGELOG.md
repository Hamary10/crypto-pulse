...
- 新增早间市场简报、晚间重点观察、每周市场复盘三套独立内容格式。
- 新增北京时间四小时栏目路由；周日 20:17 自动以周报替代晚间观察。
- dry-run 结果新增完整消息正文和静默标记。
- 周报复用 CoinGecko `/coins/markets` 免费接口的 `7d` 参数，生成近 7 日领涨和回撤复盘。
- 新增排期、周日替换、静默、边界窗口、dry-run、去重、失败重试和 7 日排序测试。
- 新增 CoinGecko 全局市场数据读取，早报展示总市值、总市值 24 小时变化、BTC 占有率和市值前 100 涨跌分布。
- 新增本地异动雷达 dry-run：BTC/ETH 阈值 ±2.5%，其他白名单主流币 ±4%，同币同方向冷却 2 小时，每小时最多 3 条。
- 新增异动雷达阈值边界、冷却边界、方向独立、小时限流、重复行、数据缺失、发送失败与 uncertain 状态测试。
- 新增 `docs/RADAR_PERSISTENCE_EVALUATION.md`，记录现有持久化缺口、生产禁用原因和未来准入条件。

### 修复

- 修复 08:17 特殊内容与普通行情可能重复规划的问题；现在每个时段只规划一个栏目。
- 修复 20:17 缺少周日优先级的问题；周日晚间只生成每周复盘。
- 修复早报、晚报信息定位相近的问题；晚报改为当前强弱、宽度和三大主流币观察。
- 修复异动雷达直接展示 CoinGecko UTC 时间的问题；现在统一转换为北京时间并明确标注。

### 优化

- 固定排期改为 00/04/08/12/16/20 点第 17 分，00:17 和 04:17 使用 Telegram 静默发送。
- 保留真实发送双门控、`:10-:45` 时间窗、进程内时段去重、免责声明和失败保护。
- 原每日涨幅榜、跌幅榜、热门榜三消息生成逻辑保留为兼容函数，不接入新自动排期。
- 周报过滤稳定币、包装币、质押凭证和明显重复锚定资产，并增加规则化周度总结。
- 所有市场状态和总结使用显式数据规则，不生成行情原因、新闻或未来事件。
- 雷达真实发送由代码常量默认硬锁，当前只允许本地 dry-run；冷却和限流仅为进程内状态。
- GitHub Actions 自动 schedule 继续停用，仅把待部署 UTC cron `17 */4 * * *` 写入注释和文档。
- 本次未修改 Bot2、GroupGuard、Render、cron-job.org、`.env` 或任何密钥，也未触发真实频道发送。
...
- 记录 Bot1 从 2026-06-10 18:17 到 2026-06-12 20:17 连续稳定运行。
- 记录每 2 小时价格播报、北京时间 08:17 早间通知和榜单通知正常。
- 明确当前结论：P0 稳定试运营第一阶段通过。
...
- 交接文档明确下一步为继续观察满 7 天、小范围测试 Bot2 命令、暂不扩展新功能。
- 记录 rejected direction：不大改播报逻辑、不新增大量功能、不更换 CoinGecko 数据源。
- 明确 Crypto Pulse 与 YouTube LoFi 完全分开，不混用路径、素材、逻辑或提示词。
