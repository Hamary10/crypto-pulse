...
def get_prices(coin_ids: List[str]) -> Dict[str, Dict[str, Any]]:
    ...
    for coin_id in missing_ids:
        ...
        normalized = {
            "coin_id": coin_id,
            "symbol": coin_id,
            "price_usd": item.get("usd", 0),
            "price_cny": item.get("cny", 0),
            "market_cap": item.get("usd_market_cap"),
            "volume_24h": item.get("usd_24h_vol"),
            "price_change_percentage_24h": item.get("usd_24h_change", 0),
        }
        result[coin_id] = normalized
        _set_cached(f"price:{coin_id}", normalized)
    return result


def get_price(coin_id: str) -> Optional[Dict[str, Any]]:
    data = get_prices([coin_id])
    if is_rate_limited(data) or is_error(data):
        return data
...
def get_markets(limit: int = 20) -> List[Dict[str, Any]]:
    data = _get(
        "/coins/markets",
        {
            ...
            "per_page": max(limit, 100),
            "page": 1,
            "sparkline": "false",
            "price_change_percentage": "24h",
        },
        f"markets:{max(limit, 100)}",
    )
    if is_rate_limited(data):
        return [{"_status": "rate_limited"}]
    if is_error(data):
...
def get_top_markets(limit: int = 10) -> List[Dict[str, Any]]:
    return get_markets(limit)[:limit]
